    ...
    for trial in range(N_trials):
        ...
        for i in range(N_online):
            ...
            for h in range(env.H - 1, -1, -1):
                new_V = np.zeros(env.S)
                for s in range(env.S):
                    Qa = env.reward[h, s] + bonus[h, s] + np.dot(trans[h, s], last_V)
                    a = np.argmax(Qa)
            ...
            for h in range(env.H):
                ...
                trans[h, s, a] = count[h, s, a] / sa_count[h, s, a]
            # update regret
            hist.append(hist[-1] + optimal_value - env.value_gen(online_policy))
        hhist.append(hist)
...
for key, policy in offline_policies.items():
    print(key)
    hhist = Hybrid_UCB_regret(env, policy, N_trials=100, N_offline=1000, N_online=20000, beta = 1 * np.sqrt(15))
    suboptimality_Noff1000[key] = hhist

with open('Regret_H3_S5_A10_seed1_OfflineN_1000.pkl', 'wb') as f:
    pickle.dump({'offline': offline_policies,
                 'regret':suboptimality_Noff1000}, f)

# different numbers of offline trajectories

suboptimality_N0 = {}
policy = 'ρ₂'
for key in [0, 500, 1000, 2000]:
    print(key)
    hhist = Hybrid_UCB_regret(env, offline_policies[policy], N_trials=100, N_offline=key, N_online=20000, beta = 1 * np.sqrt(15))
    suboptimality_N0[str(key)] = hhist

with open('Regret_H3_S5_A10_seed1_OfflineN_diff.pkl', 'wb') as f:
    pickle.dump({'offline': offline_policies,
