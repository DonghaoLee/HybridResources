...
if __name__ == '__main__':
    ...
    def Hybrid_UCB(env, offline_policy,
        ...
        for trial in tqdm(range(N_trials)):
            ...
            est_reward = np.maximum(np.zeros([env.S, env.A]), np.matmul(env.user_theta, pred_features.transpose()) - bonus) # np.zeros([env.S, env.A])
            for i in range(N_online):
                ...
                est_reward[:, a] = np.maximum(est_reward[:, a], np.matmul(env.user_theta, pred_features[a]) - bonus[:, a])
                pred_a = np.argmax(est_reward, axis = -1)
                est_opt_policy = np.zeros([env.S, env.A])
                est_opt_policy[np.arange(env.S), pred_a] = 1.
                hist.append(np.einsum('sa,sa', env.reward, optimal_policy - est_opt_policy) / env.S)
            hhist.append(hist)
    ...
    for key, policy in offline_policies.items():
        print(key)
        hhist = Hybrid_UCB(env, policy, N_trials=50, N_offline=4000, N_online=50000, beta = 10.)
        suboptimality_Noff4000[key] = hhist
    
    with open('MovieLens_seed0_OfflineN_4000.pkl', 'wb') as f:
        pickle.dump({'offline': offline_policies,
                    'suboptimality':suboptimality_Noff4000}, f)
    ...
    for key in [0, 2000, 4000, 8000]:
        print(key)
        hhist = Hybrid_UCB(env, offline_policies[policy], N_trials=50, N_offline=key, N_online=50000, beta = 10.)
        suboptimality_N0[str(key)] = hhist

    with open('MovieLens_seed0_OfflineN_diff.pkl', 'wb') as f:
